...
public:
   int minTime(vector <string> s) {
      ...
      int m = s[0].size();
      int comb = 1<<n;
      vector< vector<bool> > occ(comb, vector<bool>(m, false));
      vector<int> ans(comb);// all init to 0
      for(int bits=1; bits< comb; ++bits){
         int lans=INT_MAX;
         int last = 0;
         for(int i=0;i<n;++i)
            if((bits>>i)&1){
         ...
         ans[bits]=lans;
         occ[bits]=occ[bits^(1<<last)];
         for(int i=0;i<m;++i)
            occ[bits][i]=occ[bits][i]|(s[last][i]=='1');
      }
      return ans[(comb)-1];
   }
};
