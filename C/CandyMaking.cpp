...
class CandyMaking {
public:
   double findSuitableDensity(vector <int> v, vector <int> dw) {
      int n =v.size();
      vector<long double> rho;
      for(int i=0;i<n;++i)
         rho.push_back(dw[i]/((long double)v[i]));
      long double ans=0;
      for(int i=0;i<n;++i){
         long double lans=0;
         long double p = rho[i];
         for(int j=0;j<n;++j)
            lans+=fabs(dw[j]-v[j]*p);
         if(i==0) ans=lans;
         else ans=min(ans,lans);
      }
      return ans;
   }
};
